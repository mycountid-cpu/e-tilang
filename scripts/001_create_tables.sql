...
CREATE INDEX IF NOT EXISTS idx_tickets_petugas_id ON public.tickets(petugas_id);
...
CREATE POLICY "profiles_select_petugas" ON public.profiles
  FOR SELECT USING (
    EXISTS (
...
-- RLS Policies for violations (read-only for users, full access for petugas)
CREATE POLICY "violations_select_all" ON public.violations
  FOR SELECT USING (true);

CREATE POLICY "violations_manage_petugas" ON public.violations
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND role = 'petugas'
...
CREATE POLICY "tickets_insert_petugas" ON public.tickets
...
CREATE POLICY "tickets_update_petugas" ON public.tickets
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND role = 'petugas'
