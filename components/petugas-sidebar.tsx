...
import { LayoutDashboard, FileText, Plus, List, LogOut, ShieldCheck } from "lucide-react"
import { useAuth } from "@/contexts/auth-context"

const navigation = [
  { name: "Dashboard", href: "/petugas/dashboard", icon: LayoutDashboard },
  { name: "Buat Tilang", href: "/petugas/tickets/create", icon: Plus },
  { name: "Kelola Tilang", href: "/petugas/tickets", icon: FileText },
  { name: "Data Pelanggaran", href: "/petugas/violations", icon: List },
]
  ...
  return (
    <aside className="hidden h-full w-64 flex-col border-r border-sidebar-border bg-sidebar md:flex">
      ...
      <div className="border-b border-sidebar-border px-6 py-3">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Portal Petugas</p>
      </div>
...
export function PetugasMobileNav() {
  const pathname = usePathname()
...
export function PetugasMobileHeader() {
  const router = useRouter()
  const { signOut } = useAuth()
